"""
Killzones Filter Test

Tests ICT Killzones on top of the best combination
(RR=3.0, buffer=0.10, retest=72, micro=60, bullish-only, top4).

Killzones (UTC):
    - Asian:    00:00 - 04:00
    - London:   07:00 - 10:00   (best)
    - NY AM:    12:00 - 15:00   (best)
    - NY PM:    18:00 - 21:00
...
We test several combinations:
    ...
    - london:      only London Killzone
    - ny_am:       only NY AM Killzone
    - london_ny:   London + NY AM (ICT "golden" hours)
    - london_ny_late: London + NY AM + Late
    - all_major:   London + NY AM + NY PM
...
Usage:
    python scripts/test_killzones.py
"""
...
# Killzone definitions (UTC hours, inclusive of start, exclusive of end)
KILLZONES = {
    "asian":       (0, 4),
    "london":      (7, 10),
    "ny_am":       (12, 15),
...
# Test scenarios (list of killzone names that are allowed)
SCENARIOS = {
    "off":              None,
    "london":           ["london"],
    "ny_am":            ["ny_am"],
    "london_ny":        ["london", "ny_am"],
    "london_ny_late":   ["london", "ny_am", "late"],
    "all_major":        ["london", "ny_am", "ny_pm"],
}
...
def in_killzone(ts, killzone_names):
    """Return True if timestamp is inside any of the given killzones."""
    if killzone_names is None:
        return True
    if ts is None:
        return False
    h = ts.hour
    for name in killzone_names:
        lo, hi = KILLZONES[name]
        if lo <= h < hi:
            return True
    return False
...
def run_best(precomp, killzone_names=None):
    """Run the best combination pipeline with optional killzone filter."""
    ...
    # ---- Killzone filter ----
    if killzone_names is not None:
        filtered = []
        for c in tradable:
            entry_ts = to_utc(getattr(c, "timestamp", None))
            if entry_ts is None:
                continue
            if in_killzone(entry_ts, killzone_names):
                filtered.append(c)
        tradable = filtered
...
def main():
    ...
    for name, (lo, hi) in KILLZONES.items():
        print(f"  {name:<10} {lo:02d}:00 - {hi:02d}:00")
    print()
    ...
    for scenario_name, killzone_names in SCENARIOS.items():
        print(f"Testing scenario: {scenario_name} "
              f"(killzones={killzone_names})")

        all_trades = []
        for sym, p in precomputed.items():
            trades = run_best(p, killzone_names=killzone_names)
    ...
    header = (
        f"{'Scenario':<18} {'N':>6} {'WR':>8} {'TotalR':>9} "
    ...
    for scenario_name in SCENARIOS.keys():
        ...
        summary_rows.append({
            ...
            "killzones": ",".join(SCENARIOS[scenario_name])
                if SCENARIOS[scenario_name] else "off",
            "n": r["full"]["n"],
    ...
    out_csv = output_dir / "killzones_results.csv"
