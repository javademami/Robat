"""
Multi-Parameter Optimization — FAST VERSION

Key optimization:
    - Lower layers (structure, liquidity, sweep, MSS, displacement, OB, FVG) precomputed once per symbol
    - Retests and Micro-MSS cached per (retest_window, micro_window) combo
    - Entry + Risk + Size + Backtest only re-run per (RR, buffer) combo

This reduces detect_micro_mss calls from 128×N to 16×N, and
detect_entry_candidates from 128×N to ~N.

Runs in ~15-25 minutes for 128 combos × 4 symbols.

Usage:
    python scripts/test_forex_multi_param_fast.py
"""
...
# ============================================================
# PRECOMPUTE LOWER LAYERS (once per symbol)
# ============================================================
...
def get_retests(precomp, retest_window):
    """Cache retests per retest_window."""
    cache = precomp["retests_cache"]
    if retest_window in cache:
        return cache[retest_window]

    df_1h = precomp["df_1h"]
    ob_r = detect_ob_retests(df_1h, precomp["obs"],
                             max_bars_after_zone=retest_window)
    fvg_r = detect_fvg_retests(df_1h, precomp["fvgs"],
                               max_bars_after_zone=retest_window)
    retests = list(ob_r) + list(fvg_r)
    cache[retest_window] = retests
    return retests


def get_micro(precomp, retest_window, micro_window):
    """Cache micro-MSS per (retest_window, micro_window)."""
    cache = precomp["micro_cache"]
    key = (retest_window, micro_window)
    if key in cache:
        return cache[key]

    retests = get_retests(precomp, retest_window)
    if not retests:
        cache[key] = []
        return []
...
def build_bias_map(precomp, micro):
    """Build bias_by_timestamp for a given micro list."""
    bias_map = {}
    sw4_sorted = sorted(
        precomp["swings_4h"],
...
# ============================================================
# RUN COMBO (cached upper layers)
# ============================================================

def run_combo(precomp, target_rr, buffer_atr, retest_window, micro_window):
    """
    Run the parameter-dependent part using cached retests and micro.
    """
    micro = get_micro(precomp, retest_window, micro_window)
    if not micro:
        return []
...
def main():
    print("=" * 72)
    print("MULTI-PARAMETER OPTIMIZATION — FAST")
    print("=" * 72)
    ...
    # ---- Precompute retests and micro for all combos ----
    print("Precomputing retests and micro-MSS ...")
    for sym, p in precomputed.items():
        print(f"  {sym}:")
        for retest in RETEST_OPTIONS:
            r = get_retests(p, retest)
            print(f"    retest={retest}: {len(r)} retests")

        for retest, micro_win in itertools.product(RETEST_OPTIONS, MICRO_MSS_OPTIONS):
            m = get_micro(p, retest, micro_win)
            print(f"    retest={retest} micro={micro_win}: {len(m)} micro-MSS")
    print()
    ...
    # ---- Save ----
    output_csv = DATA_DIR / "validation" / "10y" / "multi_param_fast_results.csv"
